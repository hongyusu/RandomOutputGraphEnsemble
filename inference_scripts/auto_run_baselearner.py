...
def singleBaselearner(filename,graph_type,t,node):
  try:
    ...
    logging.info('\t--> (node)%s,(f)%s,(type)%s,(t)%s' %( node,filename,graph_type,t))
    os.system(""" ssh -o StrictHostKeyChecking=no %s 'cd %s; nohup matlab -nodisplay -nosplash -nodesktop -r "run_baselearner '%s' '%s' '%s' '0' " > /var/tmp/tmp_%s_%s_%s_baselearner' """ % (node,current_DIR,filename,graph_type,t,filename,graph_type,t) )
    logging.info('\t--| (node)%s,(f)%s,(type)%s,(t)%s' %( node,filename,graph_type,t))
    time.sleep(5)
  pass
